...
const FoodCard = ({
    ...
    return (
        <>
            ...
            {isWishlist && (
                <div 
                    className="flex items-center min-h-[100px] md:hidden border-t border-gray-200 first:border-t-0"
                    onClick={onViewDetails}
                >
                    <div className="flex flex-row sm:flex-row w-full gap-4 py-4 items-center max-h-[88px]">
                        {/* Image */}
                        <img
                            src={imgURL}
                            alt={name}
                            className="w-16 h-16 object-cover rounded-full shadow-xl sm:w-20 sm:h-20"
                        />
                        {/* Info */}
                        <div className="flex-1 flex flex-col justify-between">
                            <div className="flex flex-col gap-1 max-w-[210px]">
                                <h1 className="text-[14px] font-semibold text-gray-800 max-h-[42px] w-full">{name}</h1>
                                ...
                                <div className="flex items-center gap-2 md:hidden min-h-[20px]">
                                    <div className="flex items-center gap-2 md:hidden min-h-[20px]">
                                        ...
                                        <div className="flex flex-row gap-1 text-[#A2A2A2]  text-[12px] md:text-[16px] font-medium">
                                            ...
                                            <p>
                                                {typeof totalWishes === "number" ? totalWishes : ""} 
                                                {totalWishes === 1 
                                                    ? " like" 
                                                    : " likes"}
                                            </p>
                                ...
                                {description && (
                                    <div className="text-xs text-gray-500 max-h-[20px] truncate overflow-hidden whitespace-nowrap">
                                        {description}
                                    </div>
                                )}
                            </div>
            ...
            {/* normal card */}
            <div 
                className={`flex items-end h-100 ${(isWishlist || isVote) ? "hidden sm:hidden md:flex" : "md:flex"}`}
            >
                <div 
                ...
                >
                    {/* Inner container */}
                    <div className={`relative flex flex-col gap-4 pt-34 w-full ${(isWishlist || isVote) ? "md:min-w-50" : "min-w-63"}`}>
                        {/* Floating Image */}
                        <img
                            src={imgURL}
                            alt={name}
