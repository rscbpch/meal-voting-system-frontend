...
const CanteenPick = () => {
  ...
  const total =
    upcoming?.dish && Array.isArray(upcoming.dish) ? upcoming.dish.length : 0;
  ...
  if (
    !upcoming ||
    !Array.isArray(upcoming.dish) ||
    upcoming.dish.length === 0
  ) {
    return (
      <div className="py-10 text-gray-500 text-center">
        No upcoming menu found.
      </div>
    );
  }
  ...
  const dish = item.Dish as typeof item.Dish & {
  ...
  const imgSrc =
    dish.imageURL && dish.imageURL.trim() !== "" ? dish.imageURL : fallbackImg;
  const categoryName =
    (dish.categoryName as string | undefined) ||
              ...
              {upcoming.dish.map((_, i) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full ${
                    i === currentIndex ? "bg-[#429818]" : "bg-gray-300"
                  }`}
